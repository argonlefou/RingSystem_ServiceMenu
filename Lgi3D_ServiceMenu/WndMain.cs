using System;
using System.IO;
using System.Windows.Forms;

namespace Lgi3D_ServiceMenu
{
    public partial class WndMain : Form
    {
        //Cabinet REGION can be changed in \Fraw\fwDataDB\fwDataDB_AM.txt
        //string,DefaultRegion,,,USA
        //string,DefaultRegion,,,JPN
        //string,DefaultRegion,,,CHN
        //string,DefaultRegion,,,EXP

        //In theory, Game Data is stored in the EEPROM file like this :
        //(BaseAddres is the first byte of the data array, starting with 4bytes-checksum)
        //BaseAddress +0x10:
        //Bit 0     : Attract Sound (0/1)
        //Bit 1-3   : Difficulty (0-4)
        //Bit 4 = 1
        //Bit 5-7   : Life (0-4)
        //BaseAddress +0x22:
        //Bit 6     : Kids Mode (0/1)
        //BaseAddress +0x23&&
        //Bit 3     : Dress Code (0/1)
        //Bit 4     : 3D Mode (0/1)
        //Bit 5     : Stage Select (0/1)
        //Bit 6     : Air (0/1)
        //BaseAddress +0x24
        //Bit 2     : Advertise Air (0/1)


        //The game is overwriting user data with backup default EPROM data for GameTest options    
        //For now, we have to change the default data that will be loaded each game in \Fraw\fwSRAM\fwFormat101_EEP.bin :
        private const int BACKUP_EEPROM_DIFFICULTY_OFFSET = 0xB8;
        private const int BACKUP_EEPROM_LIFE_OFFSET = 0x100;
        private const int BACKUP_EEPROM_ADVERTISE_OFFSET = 0x094;
        private const int BACKUP_EEPROM_AIR_OFFSET = 0x43C;
        private const int BACKUP_EEPROM_ADVAIR_OFFSET = 0x4A8;
        private const int BACKUP_EEPROM_KIDSMODE_OFFSET = 0x31C;
        private const int BACKUP_EEPROM_DRESSCODE_OFFSET = 0x3AC;
        private const int BACKUP_EEPROM_3DMODE_OFFSET = 0x3F4;
        private const int BACKUP_EEPROM_STAGESELECT_OFFSET = 0x418;

        private const string JCONFIG_EEPROM_PATH = @"\sv\E2PROM.bin";
        private const string BACKUP_EEPROM_PATH = @"\data\Fraw\fwSRAM\fwFormat104_EEP.bin";
        private const string FWDATADB_TEXT_PATH = @"\data\Fraw\fwDataDB\fwDataDB_AM.txt";
        private const UInt32 JCONFIG_REGION_OFFSET = 0x0C;  //Setting this to 0 will allow the game to choose Region from \data\Fraw\fwDataDB\fwDataDB_AM.txt. Overwised, fixed to Japanese in Jconfig DLL code.
        private const UInt32 JCONFIG_CREDITS_OFFSET = 0x20;
        private const UInt32 JCONFIG_CREDITS_LENGTH = 0x1C;
        private const UInt32 JCONFIG_OPTIONS_OFFSET = 0x122A;
        private const UInt32 JCONFIG_OPTIONS_LENGTH = 0x104;
        private byte[] CRC_TABLE = new byte[] {  };

        private byte[] _JconfigEepromContent;
        private byte[] _BackupEepromContent;

        public WndMain()
        {
            InitializeComponent();
            this.Text = "Let's Go Island 3D - System Menu v" + System.Reflection.Assembly.GetEntryAssembly().GetName().Version.ToString();
        }

        private void WndMain_Load(object sender, EventArgs e)
        {
            //Backup EEPROM is part of the game installation, should be here....
            if (!File.Exists(Application.StartupPath + BACKUP_EEPROM_PATH))
            {
                _BackupEepromContent = Lgi3D_ServiceMenu.Properties.Resources.fwFormat104_EEP;
            }
            else
            {
                try
                {
                    _BackupEepromContent = File.ReadAllBytes(Application.StartupPath + BACKUP_EEPROM_PATH);
                }
                catch (Exception Ex)
                {
                    MessageBox.Show("Error reading EPROM file :\n\n" + Ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }

            if (!File.Exists(Application.StartupPath + JCONFIG_EEPROM_PATH))
            {
                _JconfigEepromContent = Lgi3D_ServiceMenu.Properties.Resources.E2PROM;
            }
            else
            {
                try
                {
                    _JconfigEepromContent = File.ReadAllBytes(Application.StartupPath + JCONFIG_EEPROM_PATH);
                }
                catch (Exception Ex)
                {
                    MessageBox.Show("Error reading EPROM file :\n\n" + Ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }

            DisplayJconfigEepromValues(_JconfigEepromContent);
            DisplayDefaultEepromValues(_BackupEepromContent);
        }

        private void DisplayJconfigEepromValues(byte[] Jconfig_Data)
        {
            //Credits in Jconfig EEPROM
            Cbox_FreePlay.SelectedIndex = Jconfig_Data[JCONFIG_CREDITS_OFFSET + 0x0A];
            Cbox_CreditStart.SelectedIndex = Jconfig_Data[JCONFIG_CREDITS_OFFSET + 0x0F] - 1;
            Cbox_CreditContinue.SelectedIndex = Jconfig_Data[JCONFIG_CREDITS_OFFSET + 0x11] - 1;
        }
        private void DisplayDefaultEepromValues(byte[] Jconfig_Data)
        {
            try
            {
                using (StreamReader Sr = new StreamReader(Application.StartupPath + FWDATADB_TEXT_PATH))
                {
                    string line = Sr.ReadLine();
                    while (line != null)
                    {
                        if (line.Contains("string,DefaultRegion,,,"))
                        {
                            Cbox_Region.Text = line.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries)[2];
                        }
                        line = Sr.ReadLine();
                    }
                }
            }
            catch (Exception Ex)
            {
                MessageBox.Show("Error reading the following file : " + FWDATADB_TEXT_PATH + "\n\n" + Ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }

            //Game option in default EEPROM
            Cbox_Difficulty.SelectedIndex = Jconfig_Data[BACKUP_EEPROM_DIFFICULTY_OFFSET];
            Cbox_Life.SelectedIndex = Jconfig_Data[BACKUP_EEPROM_LIFE_OFFSET];
            Cbox_Advertise.SelectedIndex = Jconfig_Data[BACKUP_EEPROM_ADVERTISE_OFFSET];
            Cbox_Air.SelectedIndex = Jconfig_Data[BACKUP_EEPROM_AIR_OFFSET];
            Cbox_AdvertiseAir.SelectedIndex = Jconfig_Data[BACKUP_EEPROM_ADVAIR_OFFSET];
            Cbox_KidsMode.SelectedIndex = Jconfig_Data[BACKUP_EEPROM_KIDSMODE_OFFSET];
            Cbox_DressCode.SelectedIndex = Jconfig_Data[BACKUP_EEPROM_DRESSCODE_OFFSET];
            Cbox_3D.SelectedIndex = Jconfig_Data[BACKUP_EEPROM_3DMODE_OFFSET];
            Cbox_StageSelect.SelectedIndex = Jconfig_Data[BACKUP_EEPROM_STAGESELECT_OFFSET];
        }

        private void UpdateCreditsData(UInt32 DataStartOffset)
        {
            _JconfigEepromContent[DataStartOffset + 0x0A] = (byte)Cbox_FreePlay.SelectedIndex;
            _JconfigEepromContent[DataStartOffset + 0x0F] = (byte)(Cbox_CreditStart.SelectedIndex + 1);
            _JconfigEepromContent[DataStartOffset + 0x11] = (byte)(Cbox_CreditContinue.SelectedIndex + 1);

            byte[] UncheckedData = new byte[JCONFIG_CREDITS_LENGTH];
            for (int i = 0; i < JCONFIG_CREDITS_LENGTH; i++)
            {
                UncheckedData[i] = _JconfigEepromContent[DataStartOffset + i + 4];
            }
            UInt32 Checksum = CalculateChecksum(UncheckedData);
            _JconfigEepromContent[DataStartOffset] = BitConverter.GetBytes(Checksum)[0];
            _JconfigEepromContent[DataStartOffset + 1] = BitConverter.GetBytes(Checksum)[1];
            _JconfigEepromContent[DataStartOffset + 2] = BitConverter.GetBytes(Checksum)[2];
            _JconfigEepromContent[DataStartOffset + 3] = BitConverter.GetBytes(Checksum)[3];
        }

        private void UpdateOptionsData()
        {
            _BackupEepromContent[BACKUP_EEPROM_DIFFICULTY_OFFSET] = (byte)Cbox_Difficulty.SelectedIndex;
            _BackupEepromContent[BACKUP_EEPROM_LIFE_OFFSET] = (byte)Cbox_Life.SelectedIndex;
            _BackupEepromContent[BACKUP_EEPROM_ADVERTISE_OFFSET] = (byte)Cbox_Advertise.SelectedIndex;
            _BackupEepromContent[BACKUP_EEPROM_AIR_OFFSET] = (byte)Cbox_Air.SelectedIndex;
            _BackupEepromContent[BACKUP_EEPROM_ADVAIR_OFFSET] = (byte)Cbox_AdvertiseAir.SelectedIndex;
            _BackupEepromContent[BACKUP_EEPROM_KIDSMODE_OFFSET] = (byte)Cbox_KidsMode.SelectedIndex;
            _BackupEepromContent[BACKUP_EEPROM_DRESSCODE_OFFSET] = (byte)Cbox_DressCode.SelectedIndex;
            _BackupEepromContent[BACKUP_EEPROM_3DMODE_OFFSET] = (byte)Cbox_3D.SelectedIndex;
            _BackupEepromContent[BACKUP_EEPROM_STAGESELECT_OFFSET] = (byte)Cbox_StageSelect.SelectedIndex;
        }

        private UInt32 CalculateChecksum(byte[] Data)
        {
            UInt32 Checksum = 0xFFFFFFFF;
            for (int i = 0; i < Data.Length; i++)
            {
                UInt32 a = Data[i];
                a = a ^ Checksum;
                a = a & 0x000000FF;
                Checksum = Checksum >> 8;
                Checksum = Checksum ^ BitConverter.ToUInt32(CRC_TABLE, (int)a * 4);
            }
            return ~Checksum;
        }

        bool IsBitSet(byte b, int Pos)
        {
            return (b & (1 << Pos)) != 0;
        }

        private void Btn_Save_Click(object sender, EventArgs e)
        {
            //Fix region selection
            _JconfigEepromContent[JCONFIG_REGION_OFFSET] = 0x00;
            UpdateCreditsData(JCONFIG_CREDITS_OFFSET);
            UpdateCreditsData(JCONFIG_CREDITS_OFFSET + 0x200);
            UpdateOptionsData();

            String fwDataDB_AM = String.Empty;
            try
            {
                using (StreamReader Sr = new StreamReader(Application.StartupPath + FWDATADB_TEXT_PATH))
                {
                    string Line = Sr.ReadLine();
                    while (Line != null)
                    {
                        if (Line.Contains("string,DefaultRegion,,,"))
                        {
                            string[] Buffer = Line.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                            Line = Buffer[0] + "," + Buffer[1] + ",,," + Cbox_Region.Text;
                        }

                        fwDataDB_AM += Line + Environment.NewLine;
                        Line = Sr.ReadLine();
                    }
                }
                File.WriteAllText(Application.StartupPath + FWDATADB_TEXT_PATH, fwDataDB_AM);

                DirectoryInfo di = Directory.CreateDirectory(Application.StartupPath + @"\sv");
                File.WriteAllBytes(Application.StartupPath + JCONFIG_EEPROM_PATH, _JconfigEepromContent);

                File.WriteAllBytes(Application.StartupPath + BACKUP_EEPROM_PATH, _BackupEepromContent);


                MessageBox.Show("save complete !", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception Ex)
            {
                MessageBox.Show("Error writing file :\n\n" + Ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void Btn_Close_Click(object sender, EventArgs e)
        {
            Application.Exit();
        }

        
    }
}
