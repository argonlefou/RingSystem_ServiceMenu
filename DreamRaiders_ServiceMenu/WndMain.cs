using System;
using System.IO;
using System.Windows.Forms;

namespace DreamRaiders_ServiceMenu
{
    public partial class WndMain : Form
    {
        private const string JCONFIG_EEPROM_PATH = @"\sv\E2PROM.bin";
        private const UInt32 JCONFIG_CREDITS_OFFSET = 0x20;
        private const UInt32 JCONFIG_CREDITS_LENGTH = 0x1C;
        private const UInt32 JCONFIG_OPTIONS_OFFSET = 0x1000;
        private const UInt32 JCONFIG_OPTIONS_LENGTH = 0x404;
        private byte[] CRC_TABLE = new byte[] {  };

        private byte[] _EepromContent;
        private string _CabinetRegion = String.Empty;
        private byte _LanguageIndex = 0;

        public WndMain()
        {
            InitializeComponent();
            this.Text = "SEGA Dream Raiders - System Menu v" + System.Reflection.Assembly.GetEntryAssembly().GetName().Version.ToString();                
        }

        private void WndMain_Load(object sender, EventArgs e)
        {
            if (!File.Exists(Application.StartupPath + JCONFIG_EEPROM_PATH))
            {
                _EepromContent = DreamRaiders_ServiceMenu.Properties.Resources.E2PROM;
            }
            else
            {
                try
                {
                    _EepromContent = File.ReadAllBytes(Application.StartupPath + JCONFIG_EEPROM_PATH);
                }
                catch (Exception Ex)
                {
                    MessageBox.Show("Error reading Jonfig E2PROM file :\n\n" + Ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }

            //Reading the game.ini file to get the desired Region
            //This will be important to set up language in the EPROM file
            try
            {
                using (StreamReader Sr = new StreamReader(@"..\game.ini"))
                {
                    string line = Sr.ReadLine();
                    while (line != null)
                    {
                        line = line.Replace("\t", " ");
                        string[] Buffer = line.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);

                        if (Buffer.Length > 1 && Buffer[0].ToUpper().Equals("REGION"))
                        {
                            _CabinetRegion = Buffer[1].ToUpper();
                            break;
                        }
                        line = Sr.ReadLine();
                    }
                }
            }
            catch (Exception Ex)
            {
                MessageBox.Show("Error reading game.ini file :\n\n" + Ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }            
            DisplayJconfigEepromValues(_EepromContent);            
        }

        private void DisplayJconfigEepromValues(byte[] Jconfig_Data)
        {
            Cbox_FreePlay.SelectedIndex = Jconfig_Data[JCONFIG_CREDITS_OFFSET + 0xA];
            Cbox_CreditStart.SelectedIndex = Jconfig_Data[JCONFIG_CREDITS_OFFSET + 15] - 1;
            Cbox_CreditContinue.SelectedIndex = Jconfig_Data[JCONFIG_CREDITS_OFFSET + 16] - 1;

            Cbox_Difficulty.SelectedIndex = Jconfig_Data[JCONFIG_OPTIONS_OFFSET + 0x87];
            Cbox_Language.Items.Clear();
            switch (_CabinetRegion)
            {                
                case "U":
                    {
                        Cbox_Language.Items.Add("ENGLISH");
                        Cbox_Language.SelectedIndex = 0;
                    } break;
                case "J":
                    {
                        Cbox_Language.Items.Add("JAPANESE");
                        _LanguageIndex = 1;
                        Cbox_Language.SelectedIndex = 0;
                    } break;
                case "C":
                    {
                        Cbox_Language.Items.Add("CHINESE");
                        _LanguageIndex = 2;
                        Cbox_Language.SelectedIndex = 0;
                    } break;
                case "E":
                    {
                        Cbox_Language.Items.Add("SPANISH");
                        Cbox_Language.Items.Add("RUSSIAN");
                        Cbox_Language.Items.Add("ITALIAN");
                        Cbox_Language.Items.Add("FRENCH");
                        Cbox_Language.Items.Add("TURKISH");
                        _LanguageIndex = 3;

                        if (_EepromContent[JCONFIG_OPTIONS_OFFSET + 0x11F] > _LanguageIndex)
                            Cbox_Language.SelectedIndex = _EepromContent[JCONFIG_OPTIONS_OFFSET + 0x11F] - _LanguageIndex;
                        else
                            Cbox_Language.SelectedIndex = 0;
                    } break;
                default:
                    break;
            }            
            Cbox_Subtitles.SelectedIndex = Jconfig_Data[JCONFIG_OPTIONS_OFFSET + 0x1FC];
            Cbox_LevelSelectTime.SelectedIndex = Jconfig_Data[JCONFIG_OPTIONS_OFFSET + 0xB2] / 5 - 2;
            Cbox_GameEntry.SelectedIndex = Jconfig_Data[JCONFIG_OPTIONS_OFFSET + 0x1F1];
            Cbox_Continue.SelectedIndex = Jconfig_Data[JCONFIG_OPTIONS_OFFSET + 0x9C];
            Cbox_ContinueTime.SelectedIndex = Jconfig_Data[JCONFIG_OPTIONS_OFFSET + 0xA7] / 5 - 2;
            Cbox_Advertise.SelectedIndex = Jconfig_Data[JCONFIG_OPTIONS_OFFSET + 0x34] / 5;
            Cbox_P1Recoil.SelectedIndex = Jconfig_Data[JCONFIG_OPTIONS_OFFSET + 0x1BD];
            Cbox_IoSeat.SelectedIndex = Jconfig_Data[JCONFIG_OPTIONS_OFFSET + 0x1CA];
            Cbox_AdvSeat.SelectedIndex = Jconfig_Data[JCONFIG_OPTIONS_OFFSET + 0x1D8];
            Cbox_Fan.SelectedIndex = Jconfig_Data[JCONFIG_OPTIONS_OFFSET + 0x1E4];
        }

        private void UpdateCreditsData(UInt32 DataStartOffset)
        {
            _EepromContent[DataStartOffset + 0x0A] = (byte)Cbox_FreePlay.SelectedIndex;
            _EepromContent[DataStartOffset + 0x0F] = (byte)(Cbox_CreditStart.SelectedIndex + 1);
            _EepromContent[DataStartOffset + 0x10] = (byte)(Cbox_CreditContinue.SelectedIndex + 1);

            byte[] UncheckedData = new byte[JCONFIG_CREDITS_LENGTH];
            for (int i = 0; i < JCONFIG_CREDITS_LENGTH; i++)
            {
                UncheckedData[i] = _EepromContent[DataStartOffset + i + 4];
            }
            UInt32 Checksum = CalculateChecksum(UncheckedData);
            _EepromContent[DataStartOffset] = BitConverter.GetBytes(Checksum)[0];
            _EepromContent[DataStartOffset + 1] = BitConverter.GetBytes(Checksum)[1];
            _EepromContent[DataStartOffset + 2] = BitConverter.GetBytes(Checksum)[2];
            _EepromContent[DataStartOffset + 3] = BitConverter.GetBytes(Checksum)[3];
        }

        private void UpdateOptionsData(UInt32 DataStartOffset)
        {
            //Modifying Test menu bytes
            _EepromContent[DataStartOffset + 0x87] = (byte)Cbox_Difficulty.SelectedIndex;
            //0=English
            //1=Jap
            //2=Chinese
            //3=Espagne
            //4=Russian
            //5=Italian
            //6=French
            //7=turkish
            _EepromContent[DataStartOffset + 0x11F] = (byte)(Cbox_Language.SelectedIndex + _LanguageIndex);
            _EepromContent[DataStartOffset + 0x2FB] = 0x03;
            _EepromContent[DataStartOffset + 0x1FC] = (byte)Cbox_Subtitles.SelectedIndex;
            _EepromContent[DataStartOffset + 0xB2] = (byte)((Cbox_LevelSelectTime.SelectedIndex + 2) * 5);
            _EepromContent[DataStartOffset + 0x1F1] = (byte)Cbox_GameEntry.SelectedIndex;
            _EepromContent[DataStartOffset + 0x9C] = (byte)Cbox_Continue.SelectedIndex;
            _EepromContent[DataStartOffset + 0xA7] = (byte)((Cbox_ContinueTime.SelectedIndex + 2) * 5);
            _EepromContent[DataStartOffset + 0x34] = (byte)(Cbox_Advertise.SelectedIndex * 5);
            _EepromContent[DataStartOffset + 0x1BD] = (byte)Cbox_P1Recoil.SelectedIndex;
            _EepromContent[DataStartOffset + 0x1CA] = (byte)Cbox_IoSeat.SelectedIndex;
            _EepromContent[DataStartOffset + 0x1D8] = (byte)Cbox_AdvSeat.SelectedIndex;
            _EepromContent[DataStartOffset + 0x1E4] = (byte)Cbox_Fan.SelectedIndex;

            _EepromContent[DataStartOffset + 0x227] = 0x30;
            _EepromContent[DataStartOffset + 0x295] = 0x00;


            byte[] UncheckedData = new byte[JCONFIG_OPTIONS_LENGTH];
            for (int i = 0; i < JCONFIG_OPTIONS_LENGTH; i++)
            {
                UncheckedData[i] = _EepromContent[DataStartOffset + i + 4];
            }
            UInt32 Checksum = CalculateChecksum(UncheckedData);
            _EepromContent[DataStartOffset] = BitConverter.GetBytes(Checksum)[0];
            _EepromContent[DataStartOffset + 1] = BitConverter.GetBytes(Checksum)[1];
            _EepromContent[DataStartOffset + 2] = BitConverter.GetBytes(Checksum)[2];
            _EepromContent[DataStartOffset + 3] = BitConverter.GetBytes(Checksum)[3];
        }

        private UInt32 CalculateChecksum(byte[] Data)
        {
            UInt32 Checksum = 0xFFFFFFFF;
            for (int i = 0; i < Data.Length; i++)
            {
                UInt32 a = Data[i];
                a = a ^ Checksum;
                a = a & 0x000000FF;
                Checksum = Checksum >> 8;
                Checksum = Checksum ^ BitConverter.ToUInt32(CRC_TABLE, (int)a * 4);
            }
            return ~Checksum;
        }

        private void Btn_Save_Click(object sender, EventArgs e)
        {
            UpdateCreditsData(JCONFIG_CREDITS_OFFSET);
            UpdateCreditsData(JCONFIG_CREDITS_OFFSET + 0x200);
            UpdateOptionsData(JCONFIG_OPTIONS_OFFSET);
            UpdateOptionsData(JCONFIG_OPTIONS_OFFSET + JCONFIG_OPTIONS_LENGTH + 4);

            try
            {
                DirectoryInfo di = Directory.CreateDirectory(Application.StartupPath + @"\sv");
                File.WriteAllBytes(Application.StartupPath + JCONFIG_EEPROM_PATH, _EepromContent);

                MessageBox.Show("save complete !", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception Ex)
            {
                MessageBox.Show("Error writing file :\n\n" + Ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void Btn_Close_Click(object sender, EventArgs e)
        {
            Application.Exit();
        }
    }
}
